...
ten fuller host adapters, bounded DSH capabilities, and a TraeCode
configured-assets-only adapter whose skills install by copying or linking
`skills/better-harness` into a project `.trae/skills` or `~/.trae-cn/skills`
directory. [Compare all adapter support boundaries](./hosts/adapter-matrix),
